...
class Simulator(gym.Env):
  ...
  def reset(self, seed=None): #set to 42 for the experiment
  ...
  def render(self):
    if self._render_mode == "rgb_array_list":
        ...
        if self._render_stack_pop:
            self._render_stack = []
        return render_stack, []  # return empty perception images list

    elif self._render_mode == "rgb_array":
        main_img = self._GUI_rendering()  # this should get main image with perception embedding logic removed
        perception_imgs = self.perception.get_renders() if hasattr(self, 'perception') else []
        return main_img, perception_imgs
