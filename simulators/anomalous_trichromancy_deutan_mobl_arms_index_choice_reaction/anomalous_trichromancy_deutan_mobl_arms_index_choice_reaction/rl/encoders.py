...
class HugeCNN(BaseEncoder):

  def __init__(self, observation_shape, out_features):
    super().__init__(observation_shape)
    cnn = nn.Sequential(
      nn.Conv2d(in_channels=observation_shape[0], out_channels=8, kernel_size=(3, 3), padding=(1, 1), stride=(1, 1)),
      nn.LeakyReLU(),
      nn.Conv2d(in_channels=8, out_channels=16, kernel_size=(3, 3), padding=(1, 1), stride=(1, 1)),
      nn.LeakyReLU(),
      nn.Conv2d(in_channels=16, out_channels=32, kernel_size=(3, 3), padding=(1, 1), stride=(1, 1)),
      nn.LeakyReLU(),
      nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(3, 3), padding=(1, 1), stride=(2, 2)),
      nn.LeakyReLU(),
