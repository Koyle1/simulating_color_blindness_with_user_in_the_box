...
class FixedEye(BaseModule):
  ...
  def simulate_blue_cone_monochromancy(rgb):
      ...
      # Reduce the A (green-red) and B (blue-yellow) channels to desaturate
      a[:] = 128  # Neutralize green-red
      b[:] = 128  # Neutralize blue-yellow

      # Merge back and convert to BGR
      desaturated_image = cv2.merge((l, a, b))
      desaturated_image = cv2.cvtColor(desaturated_image, cv2.COLOR_LAB2BGR)

      # Enhance the blue channel
      blue_emphasized = desaturated_image.copy()
      blue_emphasized[:, :, 0] = cv2.addWeighted(desaturated_image[:, :, 0], 1.5, np.zeros_like(desaturated_image[:, :, 0]), 0, 0)

      return blue_emphasized
